...
class FirebaseAppDistributionTesterApiClient {
  ...
  /**
   * Fetches the lastest release for the app that the tester has access to, or null if the tester
   * doesn't have access to any releases.
   */
  @Nullable
  ...
  private FirebaseAppDistributionException getExceptionForHttpResponse(
    ...
    try {
      ...
      switch (connection.getResponseCode()) {
        ...
        case 404:
          return new FirebaseAppDistributionException(
              ErrorMessages.NOT_FOUND_ERROR, Status.AUTHENTICATION_FAILURE);
        case 408:
        case 504:
          return new FirebaseAppDistributionException(
              ErrorMessages.TIMEOUT_ERROR, Status.NETWORK_FAILURE);
        default:
          // TODO: Update this status, network failure is misleading
          return new FirebaseAppDistributionException(
              ErrorMessages.NETWORK_ERROR, Status.NETWORK_FAILURE);
      }
    } catch (IOException ex) {
      // TODO: Try and avoid having to catch this here. This should be impossible since we'rve
      return new FirebaseAppDistributionException(
          ErrorMessages.NETWORK_ERROR, Status.NETWORK_FAILURE, ex);
    }
  }
